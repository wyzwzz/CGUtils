...
#include "CGUtils/math/impl/swizzle_vec3.inl"
};
...
template <typename T>
struct hash<wzz::math::tvec3<T>>{
    size_t operator()(const wzz::math::tvec3<T>& vec) const noexcept{
        return wzz::misc::hash(vec.x,vec.y,vec.z);
    }
};
